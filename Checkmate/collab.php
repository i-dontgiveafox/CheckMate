...
$teams = $conn->query("SELECT * FROM team WHERE team_creator_id = $user_id ORDER BY created_at ASC");
...
?>
<?php
// Fetch all users (put this above your HTML, where you have DB connection)
$users_query = $conn->query("SELECT user_id, user_firstname, user_lastname FROM user");
$users = [];
...
<script>
    // Make PHP $users available to JavaScript
...
<div class="wrapper">
                        ...
                        <div>
                            ...
                            <div id="memberContainer">
                                <div class="member-block" style="display:flex; gap:10px; margin-top:10px; align-items:center;">
                                    <select name="members[]" required="" style="flex: 2 1 0%; padding: 10px; border-radius: 8px; border: 1px solid rgb(204, 204, 204);">
                                        <option value="">Select member</option>
                                        <?php
                                        $users = $conn->query("SELECT user_id, user_firstname, user_lastname FROM user");
                                    ...
                                    <button type="button" class="removeMember" 
    ...
    ×
                        ...
                        <button type="submit" id="saveGroupBtn" 
    style="background-color:rgb(6, 177, 40); color: white; padding: 12px; border: none; 
        ...
        <div class="team-link">
            <div class="team-container" data-id="<?= $row['team_id'] ?>" style="background: <?= htmlspecialchars($row['team_color'] ?? '#ccc') ?>;">
                <div class="team-header" style="display:flex; justify-content:space-between;">
                    ...
                    <div style="display: block; flex-direction: column; align-items: flex-end;">
                        <button 
                            class="delete-team" 
                            data-id="<?= $row['team_id'] ?>" 
                        ...
                        >✖</button>
                        <a 
                            href="team_page.php?id=<?= $row['team_id'] ?>" 
                            class="enter-team" 
                ...
                <div class="team-members">
                    ...
                    <!-- Add Member Button -->
                    <div class="add-member-btn" data-team-id="<?= $row['team_id'] ?>" title="Add Member">+</div>
                </div>
